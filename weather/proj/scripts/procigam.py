...
def run():
    ...
    for item in arquivos:
	    ...
	    strlat = trataPonto(sheet.cell(row,11).value, sheet.cell(row,12).value, sheet.cell(row,13).value)
	    strlng = trataPonto(sheet.cell(row,14).value, sheet.cell(row,15).value, sheet.cell(row,16).value)
