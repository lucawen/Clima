...
class ProcMedicao:
    ...
    def __processaItem(self, item):
        ...
        if len(col) == 0:
            self.__adderro('Parametro nao existe',   \
                        item.param,               \
                        item.caminho )
            return False
        else:
        ...
        if len(col) == 0:
            self.__adderro('Ponto de monitoramento nao existe',   \
                            item.ponto,                            \
                            item.caminho )
            return False
        else:
        ...
        if '<' in str(item.valor):
            ...
            try:
               ...
               self.__adderro('Erro ao converter valor',   \
                            item.valor,                  \
                            item.caminho )
               return False
